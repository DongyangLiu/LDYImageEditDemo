...
//edit by ldy
        if (_square) {
            CGFloat width = fmin(self.scrollView.frame.size.width, self.scrollView.frame.size.height);
            CGFloat height = fmin(self.scrollView.frame.size.width, self.scrollView.frame.size.height);
            CGFloat originX = (self.scrollView.frame.size.width - width) / 2.0 + self.scrollView.frame.origin.x;
            CGFloat originY = (self.scrollView.frame.size.height - height) / 2.0 + self.scrollView.frame.origin.y;
            
            [self layoutCropRectViewWithCropRect:CGRectMake(originX, originY, width, height)];
        }else{
            [self layoutCropRectViewWithCropRect:self.scrollView.frame];
        }
...
//edit by ldy
    if (_square) {
        self.cropRectView.frame = CGRectMake(cropRect.origin.x, cropRect.origin.y, fmin(cropRect.size.width, cropRect.size.height), fmin(cropRect.size.width, cropRect.size.height));
    }else{
        self.cropRectView.frame = cropRect;
    }
...
//edit by ldy
    if (_square) {
        cropRect = CGRectMake(cropRect.origin.x, cropRect.origin.y, fmin(cropRect.size.width, cropRect.size.height), fmin(cropRect.size.width, cropRect.size.height));
    }
...
//edit by ldy 
- (BOOL)cropRectViewSquare:(PECropRectView *)cropRectView
{
    return _square;
}
...
- (BOOL)isCrop
{
    return (self.scrollView.zoomScale == 1) ? NO : YES;
}
@end

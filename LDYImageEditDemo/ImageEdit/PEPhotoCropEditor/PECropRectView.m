...
//edit by ldy
@property (nonatomic, strong) UILabel *rectValueLabel;
...
//edit by ldy
        self.rectValueLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 100, 30)];
        self.rectValueLabel.font = [UIFont systemFontOfSize:15];
        self.rectValueLabel.backgroundColor = [UIColor clearColor];
        self.rectValueLabel.textColor = [UIColor whiteColor];
        self.rectValueLabel.textAlignment = NSTextAlignmentCenter;
        [self addSubview:self.rectValueLabel];
    }
...
//edit by ldy
    self.rectValueLabel.text = [NSString stringWithFormat:@"(%.f,%.f)",self.frame.size.width,self.frame.size.height];
    self.rectValueLabel.center = CGPointMake(width / 2.0, height / 2.0);
...
//edit by ldy
    if ([_delegate cropRectViewSquare:self]) {
        rect = CGRectMake(rect.origin.x, rect.origin.y, fmin(rect.size.width, rect.size.height), fmin(rect.size.width, rect.size.height));
    }
    return rect;
}
